...
# --- 7. メイン画面 ---
st.title("スマートコンソーシアム・シミュレーター")

tabs = st.tabs(["📈 ダッシュボード", "💎 パレートグリッド探索", "⚖️ ティア別得失と公平性評価", "🔄 トークン融通(Sankey)", "🏫 OA転換率分析", "💾 データ連携・管理"])
...
with tabs[0]: # ダッシュボード
    ...
    with col_l:
        ...
        fig.add_trace(go.Scatter(x=[total_cost], y=[total_oa], mode='markers+text', text=["現在"], textposition="top center", marker=dict(color='blue', size=20, symbol='star')))
...
with tabs[4]: # プロファイリング（縦軸拡大機能追加）
    st.header("■機関別:OA転換状況のプロファイリング分析")
    st.write("各大学の出版規模とOA転換率の相関を可視化します。特定の範囲を詳細に見るために縦軸を調整できます。")
    
    # --- 新規追加：プロファイリング用スケールコントローラー ---
    c_p1, c_p2 = st.columns(2)
    with c_p1:
        y_prof_range = st.slider("OA転換率の表示範囲 (%)", 0, 100, (40, 80))
    with c_p2:
        st.info("💡 グラフをマウスでドラッグしてもズーム可能です。")

    try:
        fig_prof = px.scatter(df_final, x="Total_Pubs", y="OA_Rate", size="Total_OA_Pubs", color="Tier", 
                             hover_name="Entity", trendline="ols", labels={"Total_Pubs": "年間総出版数", "OA_Rate": "OA転換率 (%)"})
    except:
        fig_prof = px.scatter(df_final, x="Total_Pubs", y="OA_Rate", size="Total_OA_Pubs", color="Tier", 
                             hover_name="Entity", labels={"Total_Pubs": "年間総出版数", "OA_Rate": "OA転換率 (%)"})
    
    fig_prof.update_layout(
        height=550, 
        yaxis=dict(range=[y_prof_range[0], y_prof_range[1]]), # スライダーの値を反映
...
with tabs[5]: # データ管理（UI復旧）
    ...
    with col_up:
        ...
        if f_e and not st.session_state.master_db.empty:
            log = pd.read_csv(f_e); upd = st.session_state.master_db.merge(log[['Entity', 'Log_Count']], on='Entity', how='left')
            upd['Access'] = upd['Log_Count'].fillna(upd['Access']); st.session_state.master_db = upd.drop(columns=['Log_Count']); st.rerun()
    with col_down:
